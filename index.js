...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6285240750713:+62 852-4075-0713\n' // WhatsApp ID + phone number
            + 'END:VCARD'
...
async function starts() {
    ...
    const kapankah = ['Hari Lagi','Minggu Lagi','Bulan Lagi','Tahun Lagi']
			...
			const nomorOwner = '6285240750713@s.whatsapp.net'
	       ...
	       const pushname = client.chats.get(mek.participant) === undefined ? (client.contacts[mek.key.remoteJid].notify ? client.contacts[mek.key.remoteJid].notify : "Kak") : (client.contacts[mek.participant].notify ? client.contacts[mek.participant].notify : "Kak")
			...
			case 'stiker':
				...
				case 'hilih':
					client.updatePresence(from, Presence.composing) 
					anu = await fetchJson(`https://mhankbarbars.tech/api/hilih?teks=${body.slice(7)}`, {method: 'get'})
					reply(anu.result)
					break
				case 'ytmp3':
					client.updatePresence(from, Presence.composing) 
					...
					anu = await fetchJson(`https://mhankbarbars.tech/api/yta?url=${args[0]}&apiKey=${apiKey}`, {method: 'get'})
				...
				case 'ytsearch':
					...
					anu = await fetchJson(`https://mhankbarbars.tech/api/ytsearch?q=${body.slice(10)}&apiKey=${apiKey}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = '--------------------------\n'
			...
			case 'tagall':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mention All 〙✪══\n╠➥'+teks+'╚═〘 Gawr Gurra 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mention All 〙✪══\n╠➥'+teks+'╚═〘 Gawr Gurra 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'add':
					...
					if (args.length < 1) return reply('Yang mau di add pacarnya ya?')
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'Member Jangan sok Keras 😂', text)
					}
					break
				case 'listadmins':
				case 'adminlist':
